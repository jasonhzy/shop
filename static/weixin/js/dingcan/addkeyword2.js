...
function zsc_show(id){
	...
	if(zsc_class!='add'){
		...
		click_id = id;
		var zsc_id = id;
		var url = APP+"?g=admin&m=keyword&a=addkeyword_ajax&kid="+zsc_id;
		$.post(url,null,function(data){
			$.each(data,function(i,v){
				...
				zsc_html = "<div class=\"edit-info\">"+
						   ...
						   " src=\""+URL+"/static/weixin/images/dingcan/op-modify.png\" onclick=\"zsc_editimg(0)\"/></b>"+
						   ...
						   "<img src=\""+URL+"/static/weixin/images/dingcan/keywords_28.jpg\" class=\"add1\" "+
...
function zsc_addimg(num){
	if($('.simgcon').length<9){
		...
		var zsc_html = "<a href=\"javascript:;\" class=\"simgcon\" id=\"zsc_imgs"+num+"\">"+
					   ...
					   "<b class=\"smodify\"><img src=\""+URL+"/static/weixin/images/dingcan/op-modify.png\" onclick=\"zsc_editimg("+num+")\" />"+
...
function choseImg(){
	...
	$('.zhe').show();
	$('.imgbox a').remove();
	var url = APP+"?g=admin&m=keyword&a=allimages";
	$.post(url,null,function(data){
		$.each(data,function(i,v){
...
function delImg(num){
	...
	var url = APP+"?g=admin&m=keyword&a=delimg";
...
function btnsave(num,count){
	...
	if(ktype==1){
		...
		if(count==0){
			var url = APP+"?g=admin&m=keyword&a=addkeyword&op=add";
			$.post(url,{kename:kname,keword:kword,kecontent:kecontent,ketype:ktype},function(data){
				alert('添加成功！');
				window.location.reload();
			});
		}else{
			var url = APP+"?g=admin&m=keyword&a=addkeyword&op=update";
			var kid = $('#zsc_iid').val();
...
function zscdel(num){
	...
	var url = APP+"?g=admin&m=keyword&a=addkeyword&op=del";
