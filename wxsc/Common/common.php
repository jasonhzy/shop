...
function shuxing($val,$key){	
	$num = strstr($val,"|"); 
	if($num){
		$arr = explode("|",$val);	 
		foreach($arr as $k=>$value){
			if($k==0){
				$str = " <input type='radio' name='shuxing_$key' value='$value' checked  />".$value;		
			 }else{								
			    $str .= " <input type='radio' name='shuxing_$key' value='$value'  />".$value;	
				}
		}
		...
		return $str;		
		    ...
		    $str = " <input type='radio' name='shuxing_$key' value='$val' checked  />".$val;	
		    return $str;	
