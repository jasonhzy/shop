   ...
   public function getmenu()
   {
   	$keyword = array();
    	...
    	foreach ($topmemu as $key )
    	{
    		...
    		if(count($nextmenu)!=0)//没有下级栏目
    		{
                 ...
                 {
                 	$kk[]=array('type'=>'click','name'=>urlencode($key2['name']),'key'=>$key2['key']);
                 }
                 $keyword['button'][]=array('name'=>urlencode($key['name']),'sub_button'=>$kk);
                 $kk='';
    		}else
    		{
                $keyword['button'][]=array('type'=>'click','name'=>urlencode($key['name']),'key'=>$key['key']);
    		}
    	}
	 
         return	urldecode(json_encode($keyword));
   } 
    ...
    public function creat_custom_menu()
    {
    	...
    	if($ACCESS_LIST['access_token']!='')
    	{
		   ...
		   $str = preg_replace("#\\\u([0-9a-f]+)#ie", "iconv('UCS-2', 'UTF-8', pack('H4', '\\1'))", $data);
		 $msg=$this->curl_menu($access_token,$str);
    ...
    protected function _before_update($data = '') {
        ...
        $item_cate = $this->_mod->field('name,pid')->where(array('id'=>$data['id']))->find();
