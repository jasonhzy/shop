...
class item_orderAction extends backendAction {
    ...
    public function updateRemark(){
       ...
       public function updateorder_sumPrice(){
    	$txtSellerRemark= $_POST['order_sumPrice'];//订单总额
    	$id=$_POST['id'];//订单ID
    	$data['order_sumPrice']=$txtSellerRemark;
    	if(M('item_order')->where('id='.$id)->save($data)!==false)
    	{
    		$this->success('修改成功！');
    	}else 
	 ...
	 public function updatefreeprice(){
    	$txtSellerRemark= $_POST['freeprice'];//快递费用
    	$id=$_POST['id'];//订单ID
    	$data['freeprice']=$txtSellerRemark;
    	if(M('item_order')->where('id='.$id)->save($data)!==false)
    	{
    		$this->success('修改成功！');
    	}else 
           ...
           /* if( $addid=$mod->add($data) ){
            ...
            if(count(M('address')->where('status=1')->find())==0)
             ...
             $deliveryList=	M('delivery')->where('status=1')->order('ordid asc,id asc')->select();//快递方式
              ...
              $addressList=M('address')->where('status=1')->order('ordid asc,id asc')->select();//收货地址 
