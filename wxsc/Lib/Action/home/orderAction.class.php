...
class orderAction extends userbaseAction {
	...
	public  function checkOrder()//查看订单
	{
	  ...
	  $status=$_GET['status'];
	  $item_order=M('item_order');
	  //$order =  $item_order->query('select * from __TABLE__ where orderId="'.$orderId.' " and userId="'.$this->visitor->info['id'].'"');
	   $order=$item_order->where('orderId="'.$orderId.'" and userId="'.$this->visitor->info['id'].'"')->find();
 
	  import('Think.ORG.Kuaidicompany');// 导入快递公司	
	   $kd = new Kuaidicompany();
	   $kdgs = $kd->ckkdcm($order['userfree']);
	  ...
	  }else 
	  {
	  	...
	  	foreach ($order_details as $val)
	  	{
	  		$items= array('title'=>$val['title'],'img'=>$val['img'],'price'=>$val['price'],'quantity'=>$val['quantity'],'shuxing'=>$val['shuxing']);
		   ...
		   $data['userName']=$this->visitor->info['username'];//用户名
			...
			if($orderid=$item_order->data($data)->add())//添加订单成功
			{
				...
				foreach ($_SESSION['cart'] as $item )
				{
					...
					$orders['shuxing']=$item['attr'];//商品属性				 
					$order_detail->data($orders)->add();
				}
				
		       	$modpayset = M('setting');
				$ispay = $modpayset->where("name='ipban_switch'")->getField('data');	
				$istenpay = $modpayset->where("name='tenpay'")->getField('data');	
				$alipayhome = $modpayset->where("name='alipayhome'")->getField('data');
				if($alipayhome == "") {$alipayhome = 0;}else{$alipayhome = 1;}	
								
				$this->assign('ispay',$ispay);
				$this->assign('istenpay',$istenpay);
				$this->assign('alipayhome',$alipayhome);
				$cart->clear();//清空购物车
...
//            print_r($orders);die;
			...
			}else 
			{
			   ...
			   if($orders['supportmetho']==1){ //支付宝支付
				   
						$alipay=M('alipay')->where('id=1')->select();
						$alipay =$alipay[0];
						if($alipay['leixing']==1){
							...
							echo "<script>location.href='g_alipay/alipayapi.php?WIDseller_email=".$alipay['alipayname']."&WIDout_trade_no=".$orderId."&WIDsubject=".$orderId."&WIDprice=".$orders['order_sumPrice']."'</script>";exit;
						}
						
			   }elseif($orders['supportmetho']==3){ //财付通支付
				   
						$alipay=M('alipay')->where('id=2')->select();
						$alipay =$alipay[0];
						 ///tenpay/tenpay.php?order_no=1001255412236985&product_name=商品&order_price=1.12&remarkexplain=12121212&trade_mode=1
						if($alipay['leixing']==1){
						echo "<script>location.href='tenpay/tenpay.php?order_no=".$orderId."&product_name=".$orderId."&order_price=".$orders['order_sumPrice']."&remarkexplain=".$orders['userName']."&trade_mode=1'</script>";exit;
						}else{							
						echo "<script>location.href='tenpay/tenpay.php?order_no=".$orderId."&product_name=".$orderId."&order_price=".$orders['order_sumPrice']."&remarkexplain=".$orders['userName']."&trade_mode=2'</script>";exit; 
						}
				   
				 }elseif($orders['supportmetho']==4){ //支付宝个人收款主页收款
		...
		if(IS_POST)
		{
			...
			}elseif ($payment_id==1) //支付宝
			{
				...
				if(M('item_order')->where('userId="'.$this->visitor->info['id'].'" and orderId="'.$dingdanhao.'"')->data($data)->save())
				{
				   ...
				   if($alipay['leixing']==1){
 		    		echo "<script>location.href='wapapli/alipayapi.php?WIDseller_email=".$alipay['alipayname']."&WIDout_trade_no=".$dingdanhao."&WIDsubject=".$dingdanhao."&WIDtotal_fee=".$item_order['order_sumPrice']."'</script>";exit;
					}else{
					echo "<script>location.href='g_alipay/alipayapi.php?WIDseller_email=".$alipay['alipayname']."&WIDout_trade_no=".$dingdanhao."&WIDsubject=".$dingdanhao."&WIDprice=".$item_order['order_sumPrice']."'</script>";exit;
					}
			...
			}elseif ($payment_id==4) //支付宝个人收款主页收款
			{
				$data['supportmetho']=4;
			...
			{				
				...
				if(M('item_order')->where('userId="'.$this->visitor->info['id'].'" and orderId="'.$dingdanhao.'"')->data($data)->save())
				{
					...
					///tenpay/tenpay.php?order_no=1001255412236985&product_name=商品&order_price=1.12&remarkexplain=12121212&trade_mode=1
				    if($alipay['leixing']==1){
 		    		echo "<script>location.href='tenpay/tenpay.php?order_no=".$dingdanhao."&product_name=".$dingdanhao."&order_price=".$item_order['order_sumPrice']."&remarkexplain=".$item_order['userName']."&trade_mode=1'</script>";exit;
					}else{							
					echo "<script>location.href='tenpay/tenpay.php?order_no=".$dingdanhao."&product_name=".$dingdanhao."&order_price=".$item_order['order_sumPrice']."&remarkexplain=".$item_order['userName']."&trade_mode=2'</script>";exit; 
					}
